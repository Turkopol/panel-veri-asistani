...
if uploaded_file is not None:
    ...
    time_col = st.selectbox("⏳ Zaman değişkeni (örneğin: yıl):", columns)
    y_var = st.selectbox("🎯 Bağımlı değişken (Y):", columns)
    
    # Bağımsız değişken çoklu seçim
    x_vars = st.multiselect("📈 Bağımsız değişkenler (X):", [col for col in columns if col != y_var and col != id_col and col != time_col])
...
# Gerekli modülleri dosyanın başında import et
